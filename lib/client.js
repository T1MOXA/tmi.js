	...
	// Messages with "tmi.twitch.tv" as a prefix..
		...
		switch(message.command) {
			...
			case "USERSTATE":
				...
				const modIndex = this.moderators[channel].indexOf(this.username);

				if (modIndex === -1 && message.tags["user-type"] === "mod") {
					this.moderators[channel].push(this.username);
					this.log.info('Moder added ' + JSON.stringify({channel, username: this.username}));
				} else {
					this.moderators[channel].splice(modIndex, 1);
					this.log.info('Moder removed ' + JSON.stringify({channel, username: this.username}));
				}
		...
		switch(message.command) {
			...
			case "PRIVMSG":
				...
				if (message.tags.username === "jtv") {
					...
					} else if (msg.includes("hosting you")) { // Some is hosting the channel, but no viewer(s) count provided in the message..
...
client.prototype._sendCommand = function _sendCommand(delay, channel, command, fn) {
	...
	return new Promise((resolve, reject) => {
		...
		if (_.isNull(this.ws) || this.ws.readyState !== 1) {
			...
			return reject(new Error("Not connected to server - " + JSON.stringify({delay, channel, command})));;
		} else if (typeof delay === "number") {
			_.promiseDelay(delay).then(() => {
				reject(new Error("No response from Twitch - " + JSON.stringify({delay, channel, command})));
			});
		}
...
client.prototype._sendMessage = function _sendMessage(delay, channel, message, fn) {
	...
	return new Promise((resolve, reject) => {
		...
		if (_.isNull(this.ws) || this.ws.readyState !== 1) {
			return return reject(new Error("Not connected to server - " + JSON.stringify({delay, channel, message})));;
